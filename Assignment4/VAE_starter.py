...
def scatter_plot(latent_representations, labels):
    ...
    if(latent_representations.shape[1] > 2):
        latent_representations = TSNE(n_components=2, learning_rate='auto', init='random').fit_transform(latent_representations)
...
def Plot_Kernel(_model):
    ...
    kernels = torch.reshape(kernel_enc, shape=(30, 28, 28)).cpu().detach().numpy()
...
def display_images_in_a_row(images,file_path='./reconstructed.png', display=True):
  '''
...
class VAE_Trainer(object):
    ...
    def loss_function_train(self, recon_x, x, mu, logvar, kld_weight = 0.00008):
        # Note that this function should be modified for the VAE part.
        # KLD term should be added to the final Loss.
        
        BCE = F.mse_loss(recon_x, x)
        KLD = -0.5 * (1 + logvar - mu * mu - torch.exp(logvar)).sum()
        Loss = BCE + kld_weight*KLD
        return Loss

    def loss_function_validate(self, recon_x, x):
        # Note that this function should be modified for the VAE part.
        # KLD term should be added to the final Loss.
    ...
    def train(self, epoch):
        ...
        for batch_idx, (data, _) in tqdm(enumerate(self.train_loader), total=len(self.train_loader) ) :
            ...
            loss = self.loss_function_train(recon_batch, data, mu, logvar)
            loss.backward()
    ...
    def validate(self, epoch):
        ...
        with torch.no_grad():
            for i, (data, _) in tqdm(enumerate(self.val_loader), total=len(self.val_loader)):
                ...
                val_loss += self.loss_function_validate(recon_batch, data).item()
